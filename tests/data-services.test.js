...
function engine(t, options = {}) {
  ...
  const service = {
    ...require('../src/core/storage'), ...require('../src/core/formatting'),
    ...require('../src/core/locales'),
    fm: env.local, now: new Date(), locale: 'en_US', data: {},
    settings: {
      widget: { units: 'metric', updateLocation: '10' }, weather: { locale: '' },
      events: { selectCalendars: [], numberOfDays: '1', showTomorrow: '0', showAllDay: true, minutesAfter: '', numberOfEvents: '20' },
      reminders: { selectLists: [], showWithoutDueDate: true, showOverdue: true, todayOnly: false, numberOfReminders: '20' },
      news: { url: 'https://example.com/feed?lang=en&section=world', numberOfItems: '5' },
      covid: { country: 'USA' }
    }
  };
  for (const name of ['agenda', 'location', 'cache', 'weather', 'feeds']) {
    const file = path.join(__dirname, '../src/data/', name + '.js');
    Object.assign(service, require(file));
  }
...
function event(service, id, day = 0, hour = 16, extra = {}) {
...
function ageCaches(service, minutes) {
  for (const [key, value] of service.fm.store) {
...
test('events include the entire final requested calendar day and deduplicate occurrences', async t => {
  ...
  globalThis.CalendarEvent.between = async (start, end) => [last, last, canceled, excluded].filter(item => item.startDate >= start && item.startDate < end);
  await service.setupEvents();
  assert.deepEqual(service.data.events.map(item => item.title), ['Last evening']);
});
...
test('zero future days still includes later events today', async t => {
  ...
  globalThis.CalendarEvent.between = async (start, end) => [event(service, 'Today'), event(service, 'Tomorrow', 1)].filter(item => item.startDate >= start && item.startDate < end);
  await service.setupEvents();
  assert.deepEqual(service.data.events.map(item => item.title), ['Today']);
});
...
test('event preferences preserve ongoing, all-day, future-hour, and legacy calendar behavior', async t => {
  ...
  globalThis.CalendarEvent.between = async () => [event(service, 'Ongoing', 0, 10), event(service, 'Finished', 0, 9), event(service, 'All day', 0, 0, { isAllDay: true }), event(service, 'Tomorrow', 1)];
  await service.setupEvents();
  assert.deepEqual(service.data.events.map(item => item.title), ['Ongoing']);
...
test('reminders retain due-date ordering, selected lists, overdue, and undated controls', async t => {
  const { service } = engine(t);
  const reminder = (title, days, extra = {}) => ({ title, calendar: { identifier: 'work', title: 'Work' }, dueDate: days == null ? null : new Date(service.now.getTime() + days * 86400000), isOverdue: days < 0, ...extra });
  globalThis.Reminder.allIncomplete = async () => [reminder('Undated', null), reminder('Tomorrow', 1), reminder('Overdue', -1), reminder('Today', 0), reminder('Other', -2, { calendar: { identifier: 'other', title: 'Other' } })];
  service.settings.reminders.selectLists = ' Work ';
  service.settings.reminders.todayOnly = true;
  await service.setupReminders();
  assert.deepEqual(service.data.reminders.map(item => item.title), ['Overdue', 'Today', 'Undated']);
  service.settings.reminders.showOverdue = false;
  service.settings.reminders.showWithoutDueDate = false;
  await service.setupReminders();
  assert.deepEqual(service.data.reminders.map(item => item.title), ['Today']);
});

test('location accepts zero coordinates and falls back to administrative area', async t => {
  const { service } = engine(t);
  globalThis.Location.current = async () => ({ latitude: 0, longitude: 0 });
  globalThis.Location.reverseGeocode = async () => [{ administrativeArea: 'Equator' }];
...
test('location uses a valid stale fix offline and reports unavailable without one', async t => {
  ...
  service.fm.store.get('/library/weather-cal-location').modified = new Date(service.now.getTime() - 3600000);
  globalThis.Location.current = async () => { throw Error('Denied'); };
  globalThis.Location.reverseGeocode = async () => { throw Error('Offline'); };
  assert.equal(await service.setupLocation(), true);
  assert.equal(service.data.location.locality, 'Cached');
...
test('weather exposes the complete upstream renderer contract and reuses weather for sunrise', async t => {
  const { service, requests } = engine(t, { request: async () => weatherFixture() });
  service.data.location = { latitude: 1, longitude: 2 };
  await service.setupWeather();
  await service.setupSunrise();
  assert.equal(service.data.weather.currentTemp, 22);
  assert.equal(service.data.weather.currentDescription, 'Clear');
  assert.deepEqual(service.data.weather.forecast[7], { High: 32, Low: 19, Condition: 800 });
  assert.deepEqual(service.data.weather.hourly[7], { Temp: 27, Condition: 801 });
  assert.equal(service.data.weather.tomorrowRain, 10);
  assert.equal(service.data.weather.nextHourRain, 10);
  assert.equal(service.data.sun.sunrise, 1700000000000);
  assert.equal(service.data.sun.tomorrow, 1700086400000);
  assert.equal(requests.length, 1);
});

test('short forecast arrays produce placeholders without crashing weather or sunrise', async t => {
  const { service } = engine(t, { request: async () => ({ current: { temp: 0, weather: [] }, daily: [{ temp: { max: 0, min: -2 } }], hourly: [] }) });
  await service.setupWeather();
  await service.setupSunrise();
  assert.equal(service.data.weather.currentTemp, 0);
  assert.equal(service.data.weather.currentCondition, 100);
  assert.deepEqual(service.data.weather.forecast[7], { High: null, Low: null, Condition: 100 });
  assert.equal(service.data.weather.hourly[0].Temp, null);
  assert.equal(service.data.weather.tomorrowRain, null);
...
test('weather API errors preserve a valid stale cache', async t => {
  let response = weatherFixture();
  const { service } = engine(t, { request: async () => response });
  await service.setupWeather();
  ageCaches(service, 10);
  response = { cod: 401, message: 'Unauthorized' };
  await service.setupWeather();
  assert.equal(service.data.weather.currentTemp, 22);
  assert.equal([...service.fm.store.values()].some(item => item.value.includes('Unauthorized')), false);
});

test('weather offline uses bounded cache then renders unavailable values', async t => {
  let offline = false;
  const { service } = engine(t, { request: async () => { if (offline) throw Error('Offline'); return weatherFixture(); } });
  await service.setupWeather();
  ageCaches(service, 10);
  offline = true;
  await service.setupWeather();
  ...
  await service.setupWeather();
  assert.equal(service.data.weather.currentTemp, null);
});

test('weather cache is isolated by units, normalized locale, and coordinates', async t => {
  const { service, requests } = engine(t, { request: async url => { const value = weatherFixture(); value.current.temp = new URL(url).searchParams.get('units') === 'imperial' ? 72 : 22; return value; } });
  service.data.location = { latitude: 0, longitude: 1 };
  await service.setupWeather();
  service.settings.widget.units = 'imperial';
  await service.setupWeather();
  assert.equal(service.data.weather.currentTemp, 72);
  service.settings.weather.locale = 'fr';
  await service.setupWeather();
  assert.equal(service.data.weather.currentDescription, 'ciel clair');
  service.data.location.longitude = 2;
  await service.setupWeather();
  assert.equal(requests.length, 4);
  service.data.location.longitude = 1;
  await service.setupWeather();
  assert.equal(requests.length, 4);
});
...
test('OpenWeather normalizes device language codes and falls back to English', async t => {
  ...
  await service.setupWeather();
  assert.equal(new URL(requests.at(-1)).searchParams.get('lang'), 'pt_br');
  service.locale = 'xx-YY';
  globalThis.Device.locale = () => 'zz_ZZ';
  await service.setupWeather();
  assert.equal(new URL(requests.at(-1)).searchParams.get('lang'), 'en');
});

test('weather gracefully handles denied location and malformed responses', async t => {
  const { service } = engine(t, { request: async () => ({ message: 'Unavailable' }) });
  globalThis.Location.current = async () => { throw Error('Denied'); };
  await service.setupWeather();
  await service.setupSunrise();
  assert.equal(service.data.weather.currentTemp, null);
  assert.equal(service.data.sun.sunrise, null);
});

test('API-key validation retains the legacy fallback and saves successful endpoint', async t => {
  const { service, requests } = engine(t, { request: async url => url.includes('/3.0/') ? { cod: 401 } : weatherFixture() });
  const response = await service.getWeatherApiPath('new&key');
  assert.equal(response.current.temp, 22);
  assert.equal(new URL(requests[0]).searchParams.get('appid'), 'new&key');
...
test('RSS parses CDATA, XML entities, optional fields, and safe cache filenames', async t => {
  ...
  const { service } = engine(t, { request: async () => xml });
  await service.setupNews();
  assert.equal(service.data.news[0].title, 'Wind & rain 🌧 ☂');
  assert.equal(service.data.news[0].link, 'https://example.com/a?x=1&y=2');
  assert.equal(new Date(service.data.news[0].date).toISOString(), '2026-09-08T10:00:00.000Z');
  assert.equal(service.data.news[1].title, 'Untitled update');
  assert.equal(service.data.news[1].date, null);
  for (const filename of service.fm.store.keys()) assert.equal(filename.slice('/library/'.length).includes('/'), false);
});
...
test('Atom handles namespaces, alternate links, single-quoted attributes and updated dates', async t => {
  ...
  const { service } = engine(t, { request: async () => xml });
  await service.setupNews();
  assert.equal(service.data.news.length, 1);
  assert.equal(service.data.news[0].title, 'Title "one"');
  assert.equal(service.data.news[0].link, 'https://example.com/story/1');
  assert.equal(new Date(service.data.news[0].date).toISOString(), '2026-09-08T10:00:00.000Z');
});
...
test('news fallback preserves cache on invalid responses and applies changed item limits', async t => {
  ...
  await service.setupNews();
  assert.equal(service.data.news.length, 2);
  ageCaches(service, 10);
  xml = '<html>Service unavailable</html>';
  await service.setupNews();
  assert.deepEqual(service.data.news.map(item => item.title), ['First', 'Second']);
  service.settings.news.numberOfItems = '1';
  await service.setupNews();
  assert.equal(service.data.news.length, 1);
});
...
test('COVID preserves upstream numeric fields, scopes countries, and supports global data', async t => {
  const { service, requests } = engine(t, { request: async url => ({ cases: url.includes('/all') ? 200 : 100, todayCases: 0, totalTests: 42, updated: 1700000000000 }) });
  await service.setupCovid();
  assert.equal(service.data.covid.cases, 100);
  assert.equal(service.data.covid.todayCases, 0);
  assert.match(requests[0], /^https:\/\/disease\.sh\/v3\/covid-19\/countries\/USA$/);
  service.settings.covid.country = 'World';
  await service.setupCovid();
  assert.equal(service.data.covid.cases, 200);
  assert.match(requests[1], /\/all$/);
});

test('COVID aliases disease.sh fields and keeps old cache when refresh returns an error', async t => {
  let response = { cases: 100, tests: 42, testsPerOneMillion: 12, critical: 2 };
  const { service } = engine(t, { request: async () => response });
  await service.setupCovid();
  assert.equal(service.data.covid.totalTests, 42);
  assert.equal(service.data.covid.testsPerOneMillion, 12);
...
test('truncated news feed preserves the previous headlines', async t => {
  ...
  await service.setupNews();
  assert.deepEqual(service.data.news.map(item => item.title), ['Saved']);
});
...
test('long feed URLs remain isolated without exceeding filesystem filename limits', async t => {
  const { service } = engine(t, { request: async url => '<rss><channel><item><title>' + (url.endsWith('a') ? 'A' : 'B') + '</title></item></channel></rss>' });
  service.settings.news.url = 'https://example.com/' + 'section/'.repeat(100) + 'a';
  await service.setupNews();
  assert.equal(service.data.news[0].title, 'A');
  service.settings.news.url = service.settings.news.url.slice(0, -1) + 'b';
  await service.setupNews();
  assert.equal(service.data.news[0].title, 'B');
  for (const file of service.fm.store.keys()) assert.ok(file.length < 255);
});
...
test('provider cache identity rejects unrelated envelopes', async t => {
  ...
  const { service } = engine(t, { request: async () => { if (offline) throw Error('Offline'); return weatherFixture(); } });
  await service.setupWeather();
  const [path, stored] = [...service.fm.store].find(([path]) => path.includes('cache'));
...
test('COVID supports an explicit compatible endpoint without mixing its country cache', async t => {
  const { service, requests } = engine(t, { request: async url => ({ cases: url.includes('example.com') ? 25 : 100, tests: 12 }) });
  await service.setupCovid();
  service.settings.covid.apiUrl = 'https://example.com/covid/snapshot.json';
  await service.setupCovid();
  assert.equal(service.data.covid.cases, 25);
  assert.equal(requests.at(-1), 'https://example.com/covid/snapshot.json');
...
test('stored iCloud API keys and endpoint preferences hydrate before weather requests', async t => {
  const { service, cloud, requests, downloads } = engine(t, { iCloud: true, request: async () => weatherFixture() });
  service.fm = cloud;
  ...
  await service.setupWeather();
  assert.equal(service.data.weather.currentTemp, 22);
  assert.equal(new URL(requests.at(-1)).searchParams.get('appid'), 'cloud-key');
...
test('RSS preserves encoded angle brackets as headline text while removing actual markup', async t => {
  ...
  const { service } = engine(t, { request: async () => xml });
  await service.setupNews();
  assert.deepEqual(service.data.news.map(item => item.title), ['Why 3 < 5 and 8 > 6 matters', 'Weather & traffic']);
});
...
test('Atom distinguishes literal text titles from encoded HTML titles', async t => {
  ...
  const { service } = engine(t, { request: async () => xml });
  await service.setupNews();
  assert.deepEqual(service.data.news.map(item => item.title), ['About <em> tags', 'Weather today']);
});
